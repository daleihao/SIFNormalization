...
for name_i = 1:3
    ...
    for cycle_i = 1:cycle_n
        ...
        scale_factor = PAR(filters)/100;
      
        nSIF760_true(filters) = scale_factor .* mean(rSIF760(filters_nadir));
        nSIF760_NIR(filters) = scale_factor .* mean(Refl_NIR(filters_nadir))./Refl_NIR(filters).*rSIF760(filters);
        nSIF760_NIRv(filters) = scale_factor .* mean(NIRv(filters_nadir))./NIRv(filters).*rSIF760(filters);
        %nSIF760_EVI2(filters) = mean(EVI2(filters_nadir))./EVI2(filters).*rSIF760(filters);
        
        nSIF687_true(filters) = scale_factor .* mean(rSIF687(filters_nadir));
        nSIF687_Red(filters) = scale_factor .* mean(Refl_Red(filters_nadir))./Refl_Red(filters).*rSIF687(filters);
        nSIF687_Redv(filters) = scale_factor .* mean(Redv(filters_nadir))./Redv(filters).*rSIF687(filters);
        ...
        nSIF760_Kernel(filters) = scale_factor .* c_factor.*rSIF760(filters);
        ...
        c_factor = CalculateCorrectionFactor(SZA(filters), VZA(filters), RAA(filters), Kparms);
        nSIF687_Kernel(filters) = scale_factor .* c_factor.*rSIF687(filters);
        ...
        rSIF760(filters) = scale_factor .* rSIF760(filters);
        rSIF687(filters) = scale_factor .* rSIF687(filters);
        % NIR
   ...
   %% save 
    save(['results/' nameStrings{name_i} '.mat']);
end