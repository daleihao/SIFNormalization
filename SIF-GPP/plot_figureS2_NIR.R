...
# import data
all_datas <- readMat('results/CVs_wheat_nir.mat')

all_data <- as.data.frame(all_datas['CVs.far'])  
colnames(all_data) <- c('Raw-Data','Nadir-NIR', 'Nadir-KD', 'Total-FPAR', expression(Total-i[0]))
...
plot_wheat_far <- ggplot(all_data, mapping = aes(x = variable, y = value, fill = variable)) + ylab(expression(CV)) +  ggtitle('Wheat - Far-red SIF') + 
  ...
  scale_x_discrete(labels = c('Raw-Data','Nadir-NIR', 'Nadir-KD', 'Total-FPAR', expression(Total-i[0]))) + 
...
colnames(all_data) <- c('Raw-Data','Nadir-Red', 'Nadir-KD', 'Total-FPAR', expression('Total-i'[0]))
all_data <- melt(all_data)
...
plot_wheat_red <- ggplot(all_data, mapping = aes(x = variable, y = value, fill = variable)) + ylab(expression(CV)) +  ggtitle('Wheat - Red SIF') + 
  ...
  stat_summary(fun.y=mean, geom="point", color = 'white', alpha = 1, size=2, show.legend = F) +  
  scale_fill_brewer(palette="Dark2") + 
  scale_x_discrete(labels = c('Raw-Data','Nadir-Red', 'Nadir-KD', 'Total-FPAR', expression(Total-i[0]))) + 
  #scale_color_brewer(palette="Dark2") + 
  theme_classic() + 
  theme(plot.title = element_text(face="bold", color="black",size=15, angle=0),
        axis.title.x=element_blank(),
...
# import data
all_datas <- readMat('results/CVs_corn_nir.mat')
...
plot_figure
ggsave("plot/figure_S2_nir_reds.tiff", plot = plot_figure, width = 25, height = 15, units = "cm", dpi = 600, limitsize = FALSE, compression = "lzw")