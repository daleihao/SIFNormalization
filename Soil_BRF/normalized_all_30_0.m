...
angles_sample3 = [1:9
    ...
    45*ones(1,9)
    150:-10:70
    103 97 76 63 56 49 42 35 28];


%% import hyspectral data
count = 9;
data_sample3;
allreflectance = allreflectance(:,2:end);
angles_site = angles_sample3;
for i = 1:9
    allreflectance(:,i) = smoothdata(allreflectance(:,i),'sgolay',31,'Degree',2);
end
%allreflectance(allreflectance<0.35)=nan;
...
SZAs = angles_site(2,:);
VZAs = angles_site(3,:);
phase_angles = angles_site(5,:);

SZAs = abs(SZAs')*pi/180;
VZAs = abs(VZAs')*pi/180;
phase_angles = abs(phase_angles')*pi/180;

cos_RAAs = (cos(phase_angles)-cos(SZAs).*cos(VZAs))./(sin(SZAs).*sin(VZAs));
cos_RAAs(cos_RAAs>1) = 1;
cos_RAAs(cos_RAAs<-1) = -1;
...
SZA_n = ones(count,1)*30*pi/180;
VZA_n = ones(count,1)*0;
RAA_n = ones(count,1)*0;
...
K3_n = SZA_n.*VZA_n.*cos(RAA_n);

%% modified Walthall
K0 = ones(count, 1);
K1 = SZAs.^2 + VZAs.^2;
K2 = (SZAs.^2).*(VZAs.^2);
K3 = SZAs.*VZAs.*cos(RAAs);
...
for i = 1:bandnum
    ...
    filters = Refl_m>0;
    %% RTLT kernel
    Kparms= [Kiso(filters) Kvol(filters) Kgeo(filters)]\Refl_m(filters);
    ...
    %% modified Walthall
    Kparms= [K0(filters) K1(filters) K2(filters) K3(filters)]\Refl_m(filters);
    ...
    beta0 = [0.5 0.5 0.5];
    X = [SZAs(filters) VZAs(filters) RAAs(filters)];
    y = Refl_m(filters);
    opts = statset('Display','off','TolFun',1e-15);
    mdl = fitnlm(X,y,@RPV_kernel,beta0,'Options',opts);
    
    %x2 = lsqcurvefit(@RPV_kernel,beta0,X,y,[0 0 0],[1 1 1]);
    %x3 = lsqcurvefit(@RTLT_model,beta0,X,y,[-1 -1 -1],[1 1 1]);
    
    Refl_e = Refl_m;
    Refl_e(filters) = mdl.Fitted;
...
% 1
subplot('Position', [0.1 0.55 0.37 0.4])
hold on
...
legend('RTLT','MWM','MRPV','Location','northeast')
set(gca,'fontsize',13,'linewidth',1)
ylabel('RMSE')
set(gca,'xtick',[11:100:311], 'xticklabel',[],'fontsize',15,'linewidth',1)
box
text(10,0.06*0.1,'(b)','fontsize',20)

% 3

subplot('Position', [0.1 0.12 0.37 0.4])
hold on
plot(stds(:,1),'k','linewidth',1)
plot(stds_n(:,1),'r','linewidth',1)
plot(stds_n(:,2),'g','linewidth',1)
plot(stds_n(:,3),'b','linewidth',1)
legend('Observed','RTLT','MWM','MRPV','Location','northeast')

xlabel('Wavelength (nm)')
ylabel('Std')
box
set(gca,'xtick',[11:100:311], 'xticklabel',[500:500:2000],'fontsize',15,'linewidth',1)
text(10,0.08*0.1,'(c)','fontsize',20)
% 4
...
box
set(gca,'xtick',[11:100:311], 'xticklabel',[500:500:2000],'fontsize',15,'linewidth',1)
text(10,0.2*0.1,'(d)','fontsize',20)

print(gcf, '-dtiff', '-r300', 'figure/lab_plot_30_0.tif')
...
text(0.36,0.49,['R^2 = ' num2str(R2,'%4.2f')],'fontsize',13)
text(0.36,0.47,['RMSE = ' num2str(RMSE,'%4.3f')],'fontsize',13)
...
print(gcf, '-dtiff', '-r300', 'figure/lab_scatter_30_0.tif')
...
imagesc(allreflectance_n_2', [0.3 0.5])
...
print(gcf, '-dtiff', '-r300', 'figure/ce_plot_site1_30_0.tif')
...
print(gcf, '-dtiff', '-r300', 'figure/ce_scatter_site1_30_0.tif')
...
print(gcf, '-dtiff', '-r300', 'figure/ce_image_site1_30_0.tif')
...
box
set(gca,'xtick',[11:100:311], 'xticklabel',[500:500:2000],'fontsize',15,'linewidth',1)
text(10,0.01*0.1,'(c)','fontsize',20)
ylim([0 0.01])
% 4
subplot('Position', [0.58 0.12 0.37 0.4])
hold on
...
print(gcf, '-dtiff', '-r300', 'figure/ce_plot_site2_30_0.tif')
...
print(gcf, '-dtiff', '-r300', 'figure/ce_scatter_site2_30_0.tif')
...
% 1
subplot('Position', [0.05 0.56 0.45 0.36])
imagesc(allreflectance', [0.04 0.18])
set(gca, 'xticklabel',[],'fontsize',15,'linewidth',1)
title('(a) Observed')
% 2

subplot('Position', [0.52 0.56 0.45 0.36])
imagesc(allreflectance_n_1', [0.04 0.18])
set(gca, 'xticklabel',[],'yticklabel',[],'fontsize',15,'linewidth',1)
colorbar
title('(b) RTLT')
% 3
...
subplot('Position', [0.05 0.13 0.45 0.36])
imagesc(allreflectance_n_2', [0.04 0.18])
set(gca,'fontsize',15,'linewidth',1,'xtick',[11:100:311], 'xticklabel',[500:500:2000])
xlabel('Wavelength (nm)')
title('(c) MWM')
...
subplot('Position', [0.52 0.13 0.45 0.36])
imagesc(allreflectance_n_3', [0.04 0.18])
colorbar
set(gca, 'yticklabel',[],'xtick',[11:100:311], 'xticklabel',[500:500:2000],'fontsize',15,'linewidth',1)
title('(d) MRPV')
xlabel('Wavelength (nm)')

print(gcf, '-dtiff', '-r300', 'figure/ce_image_site2_30_0.tif')
