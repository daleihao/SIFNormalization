...
for name_i = 1:3
    ...
    for cycle_i = 1:cycle_n
        ...
        filters_nadir = Cycle_Num == cycle_i  & SIF687>0.2 & SIF760>0.2; ;%& VZA == 0 
...
%                 num_noon = 7;
%         end
        filters_noon = randsample(counts,16);
        %filters_noon = Cycle_Num == 7;% & rSIF687>0.2 & rSIF760>0.2;
        [Kparms, R2, RMSE] = kernelParameterRetrieval(SZA(filters_noon), VZA(filters_noon), RAA(filters_noon), rSIF760(filters_noon),[nameStrings{name_i} '_760_' num2str(cycle_i)], 0);
        ...
        [Kparms, R2, RMSE] = kernelParameterRetrieval(SZA(filters_noon), VZA(filters_noon), RAA(filters_noon), rSIF687(filters_noon), [nameStrings{name_i} '_687_' num2str(cycle_i)], 0);
        c_factor = CalculateCorrectionFactor(SZA(filters), VZA(filters), RAA(filters), Kparms);
   ...
   %% save 
    save(['results/16_noon_' nameStrings{name_i} '.mat']);
end