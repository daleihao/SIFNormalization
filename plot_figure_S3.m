...
for i = 1:3
    ...
    hold on
   % filters = rMAE760(:,1)<1;
    plot(DOY_mean, CV_760(:,1) *100, 'ko-' , 'MarkerFaceColor', 'k')
  %  filters = rMAE760(:,2)<1;
   % scatter(DOY_mean(filters), rMAE760(filters,2) *100, 'go', 'filled')
   % filters = rMAE760(:,3)<1;
    plot(DOY_mean, CV_760(:,3) *100, 'bo-', 'MarkerFaceColor', 'b')
   % filters = rMAE760(:,4)<1;
    plot(DOY_mean, CV_760(:,4) *100, 'ro-', 'MarkerFaceColor', 'r')
    
    title(nameStrings{i}, 'FontWeight', 'Bold')
    if(i==1)
        ylabel('CV (%) for far-red SIF')
    end
    if(i==2)
         legend({'Raw Data','NIRv/Redv-based','KD-based'}, 'Location', 'Northeast')       
    end
    ...
    box on
    hold on
    plot(DOY_mean, CV_687(:,1) *100, 'ko-', 'MarkerFaceColor', 'k')
    %scatter(DOY_mean, rMAE687(:,2) *100, 'go', 'filled')
    plot(DOY_mean, CV_687(:,3) *100, 'bo-', 'MarkerFaceColor', 'b')
    plot(DOY_mean, CV_687(:,4) *100, 'ro-', 'MarkerFaceColor', 'r') %R2_687*100,
    xlabel('Hour of Day')
    if(i==1)
        ylabel('CV (%) for red SIF')
...
print(gcf, '-dtiff', '-r600', ['../writting/figure/figureS3.tif'])
close all